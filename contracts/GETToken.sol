 ...
 */
contract GETToken is PausableToken, MintableToken {
    using SafeMath for uint256;
    ...
    function GETToken (
        ...
        uint icoStartTime,
        uint icoLastingDate,
        uint icoTotalSupplyLimit,
        uint icoTokensPerEther
    ) {
        name = tokenName;                                           //set up name for display purposes
        symbol = tokenSymbol;                                       //set up symbol for display purposes
     ...
     */
    function () payable whenNotPaused preSaleActive {
        sellTokens();
    }
    ...
    function sellTokens() payable whenNotPaused preSaleActive {
        ...
        require(totalSupply.add(tokens) <= totalSupplyLimit);
